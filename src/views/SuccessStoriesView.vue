...
async function loadItems(p = 1) {
  loading.value = true
  try {
    const data = await getKnowledgeArtifacts({ category: 'Success Story', page: p, page_size: PAGE_SIZE })
      ...
      <div class="container">
        ...
        <div v-else>
          <div v-if="items.length === 0" class="text-center py-5">
            ...
            <p class="text-muted">No success stories available yet. Check back soon.</p>
          </div>
          ...
          <div v-else class="row g-4">
            ...
            >
              <div class="card h-100 border-0 shadow-sm">
                <img :src="getImageUrl(item.thumbnail_image)" class="card-img-top" style="height: 180px; object-fit: cover;" :alt="item.title" />
                <div class="card-body">
                  ...
                  <p v-if="item.author" class="small text-muted"><i class="bi bi-person me-1"></i>{{ item.author }}</p>
                </div>
                <div class="card-footer bg-transparent border-0 pb-3">
                  <RouterLink :to="{ path: '/success-details', query: { id: item.name } }" class="btn btn-sm btn-outline-success">
                    Read Story
                  </RouterLink>
                </div>
              </div>
